...
function sendCode() {
    ...
    window.authFuncs.signInWithPhoneNumber(window.auth, phoneFormatted, window.recaptchaVerifier)
        .then(result => {
...
/* Firebase 준비 후 reCAPTCHA 초기화 */
if (window.auth && window.authFuncs) initRecaptcha();
else window.addEventListener('firebase-ready', initRecaptcha);
